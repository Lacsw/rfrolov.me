...
function ProjectCard({ project, index, large = false }: { project: TProject; index: number; large?: boolean }) {
  return (
    <motion.div
      ...
      className={`group rounded-lg border bg-background p-6 transition-all duration-300 hover:shadow-sm hover:scale-[1.01] ${
        large ? "md:col-span-2" : ""
    ...
    >
      <div className={`flex flex-col h-full ${large ? "md:flex-row md:gap-8" : "gap-3"}`}>
          ...
          <div className="flex items-start justify-between gap-4">
            <div className="space-y-1.5">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className={`text-xs px-2 py-0.5 rounded ${CATEGORY_COLORS[project.category]}`}>
        ...
        <div className={`flex flex-col gap-3 ${large ? "md:items-end md:justify-between" : ""}`}>
          <div className={`flex flex-wrap gap-2 ${large ? "md:flex-col md:items-end md:gap-1.5" : ""}`}>
